...
pub fn main() {
    ...
    let mut serialized_acc = sp1_zkvm::io::read::<Vec<u8>>();
    let reader = Cursor::new(serialized_acc);
    let mut acc = rustreexo::accumulator::pollard::Pollard::deserialize(reader).unwrap();
